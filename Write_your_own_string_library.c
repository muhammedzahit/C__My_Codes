...
int returnInt(char *text); 
double returnDouble(char *text);
int main(){
    /*
    str_len : find length of a string  
    str_cpy : copy string to another string
    str_cat : add string to another string
    str_cmp : compare two strings
    returnInt : return string to int
    returnDouble : return string to double
    */
    char text[10];
    char text2[20];
    puts("Enter text 1");
...
int returnInt(char *text){
    if (str_len_double(text) == 0){
        if (*text == '-'){
            char *textPtr = text+1;
            return -1*stringToInteger(textPtr);
        }
        else{
        ...
        printf("The value unable to return");
        return 0;
    }
}
double returnDouble(char *text){
    if(str_len_double(text) >= 0) {
        if (*text == '-'){
            char *textPtr = text+1;
            return -1.0*stringToDouble(textPtr);
        }
        else {
